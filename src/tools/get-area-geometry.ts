...
export const getAreaGeometryInputSchema = {
  areaId: z.string().min(1).describe('The unique NVR area identifier (from list_protected_areas)'),
  status: z
    .string()
    .default('Gällande')
    .optional()
    .describe(
      "Decision status: 'Gällande' (valid/active), 'Överklagat' (under appeal), or 'Beslutat' (decided). Default: 'Gällande'",
    ),
};
...
export const getAreaGeometryHandler = withErrorHandling(async (args: GetAreaGeometryInput) => {
